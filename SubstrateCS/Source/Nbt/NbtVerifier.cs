...
namespace Substrate.Nbt
{
    ...
    public class NbtVerifier
    {
        ...
        private bool Verify (TagNode parent, TagNode tag, SchemaNode schema)
        {
            ...
            if (scaler != null) {
                if(VerifyScaler(tag, scaler))
                {
                    return true;
                } else { Console.WriteLine("Failed to Verify Scalar {0} with value {1}", schema.Name, tag); return false; }
            }

            SchemaNodeString str = schema as SchemaNodeString;
            if (str != null) {
                if(VerifyString(tag, str))
                {
                    return true;
                }
                else { Console.WriteLine("Failed to Verify String {0} with value {1}", schema.Name, tag); }
                return false;
            }

            SchemaNodeArray array = schema as SchemaNodeArray;
            if (array != null) {
                if (VerifyArray(tag, array))
                {
                    return true;
                }
                else
                {
                    //Console.WriteLine("Failed to Verify Array {0} with value {1}", schema.Name, tag);
                    return false;
                }
            }
            ...
            if (intarray != null) {
                if (VerifyIntArray(tag, intarray))
                {
                    return true;
                }
                else
                {
                    Console.WriteLine("Failed to Verify IntArr {0} with value {1}", schema.Name, tag);
                    return false;
                }
            }
            ...
            if (longarray != null) {
                if (VerifyLongArray(tag, longarray))
                {
                    return true;
                }
                else
                {
                    Console.WriteLine("Failed to Verify LongArr {0} with value {1}", schema.Name, tag);
                    return false;
                }            }

            SchemaNodeShortArray shortarray = schema as SchemaNodeShortArray;
            if (shortarray != null) {
                if (VerifyShortArray(tag, shortarray))
                {
                    return true;
                }
                else
                {
                    Console.WriteLine("Failed to Verify ShortArr {0} with value {1}", schema.Name, tag);
                    return false;
                }            }

            SchemaNodeList list = schema as SchemaNodeList;
            if (list != null) {
                if (VerifyList(tag, list))
                {
                    return true;
                }
                else
                {
                    //Console.WriteLine("Failed to Verify List {0} with value {1}", schema.Name, tag);
                    return false;
                }            }

            SchemaNodeCompound compound = schema as SchemaNodeCompound;
            if (compound != null) {
                if (VerifyCompound(tag, compound))
                {
                    return true;
                }
                else
                {
                    //Console.WriteLine("Failed to Verify Compound {0} with value {1}", schema.Name, tag);
                    return false;
                }            }
            Console.WriteLine("Reached invalid tag type with {0} as {1}", schema.Name, tag);
